...
{
    [HttpGet, HttpPost, Route("~/connect/external/callback/{provider:}")]
    ...
    public async Task<IActionResult> ExternalLoginCallbackAsync(string provider)
    {
        ...
        // anti-forgery stack requires at least a name claim to bind CSRF cookies to the user's identity) but
        ...
        // If the properties are missing, the best we can do is to redirect the user agent to the root URL.
    }
}